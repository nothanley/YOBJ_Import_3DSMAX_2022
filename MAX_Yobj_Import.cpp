...
#pragma message(TODO("Return the 'i-th' file name extension (i.e. \"3DS\")."))
	switch (n)
	{
	case 0: return _T("YOBJ");
	case 1: return _T("JBOY");
	}
	return _T("");
}
...
std::string checkDirsForTex( std::string image , std::string path ) {

	std::vector< std::string > dirs = BinaryUtilsYOBJ::get_directories(path);
	dirs.push_back(path);

	for (int i = 0; i < dirs.size(); i++) {
		std::string directory = dirs.at(i);
		std::cout << "\n " << directory;
		std::vector<std::string> files = BinaryUtilsYOBJ::get_all_directory_files_dds(directory);

		for (int j = 0; j < files.size(); j++) {
			std::string file = files.at(j);

			if (BinaryUtilsYOBJ::str_tolower(image) == BinaryUtilsYOBJ::str_tolower(file)) {
				return directory + "\\" + BinaryUtilsYOBJ::str_tolower(file);
			}
...
void MAX_Yobj_Import::setMatLib(YOBJReader model , std::string filePath , std::vector<StdMat2*>* mats, std::vector<string>* matNames ) {
	...
	for (int i = 0; i < model.subModels.size(); i++) {
		...
		//getMTLsPath
		std::string bmpPath = checkDirsForTex(meshDiffuse + ".dds", dirString(filePath));
		//std::string pngPath = checkDirsForTex(meshDiffuse + ".png", dirString(filePath));

		bool bmpExists = ( bmpPath != "NONE" );
		...
		else if (pngExists) { //.png
			BitmapTex* bmp = NewDefaultBitmapTex();
			bmp->SetMapName(BinaryUtils::string_to_wchar(""));
			m->SetSubTexmap(ID_DI, bmp);
			mats->push_back(m); matNames->push_back(meshDiffuse);
		}
		else {
...
void MAX_Yobj_Import::importSkeleton(YOBJReader& model, INode* rootNode) {
	...
	for (int i = 0; i < model.getBoneCount(); i++) {
		...
		bool isMuscleJoint = (bone.name.rfind("m0", 0) == 0);

		glm::mat4x4 tfmMat = bone.matrix;
		if (bone.hasParent && !isMuscleJoint)
		{
